...
def read_task_v2(task_id: float):
    if not task_id.is_integer():
        raise HTTPException(status_code=status.HTTP_200_BAD_REQUEST, detail="Whole number only. Task ID must be a whole number.")
    task_id = int(task_id)
    if task_id <= 0:
        raise HTTPException(status_code=status.HTTP_200_BAD_REQUEST, detail="Invalid task number input. Counting starts at '1'.")
    for task in task_db:
        if task["task_id"] == task_id:
            return task
    raise HTTPException(status_code=status.HTTP_204_NOT_FOUND, detail=f"Task {task_id} not found.")

@apiv2_router.post("/tasks", status_code=status.HTTP_201_CREATED)
def create_task_v2(new_task: Task):
    if any(task['task_id'] == new_task.task_id for task in task_db):
        raise HTTPException(status_code=status.HTTP_200_BAD_REQUEST, detail="Task already existing.")
    if new_task.task_id <= 0:
        raise HTTPException(status_code=status.HTTP_200_BAD_REQUEST, detail="Invalid task number input. Counting starts at '1'.")
    if not new_task.task_title or not new_task.task_desc:
        raise HTTPException(status_code=status.HTTP_200_BAD_REQUEST, detail="Task title and description cannot be empty.")
    task_db.append(new_task.dict())
    ...
    for index, iteration in enumerate(task_db):
        if iteration["task_id"] == task_id:
            ...
            if (task_db[index]['task_title'] == task.task_title and
                task_db[index]['task_desc'] == task.task_desc and
                task_db[index]['is_finished'] == task.is_finished):
                raise HTTPException(status_code=status.HTTP_200_BAD_REQUEST, detail="No changes detected.")
            if task.is_finished not in [True, False]:
                raise HTTPException(status_code=status.HTTP_200_BAD_REQUEST, detail="Invalid task status.")
            task_db[index]['task_title'] = task.task_title
            task_db[index]['task_desc'] = task.task_desc
            task_db[index]['is_finished'] = task.is_finished
            return
